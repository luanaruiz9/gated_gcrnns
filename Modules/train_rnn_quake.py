...
def MultipleModels(modelsDict, data, nEpochs, batchSize, seqLen, stateFeat, rnnStateFeat, **kwargs):
    ...
    for epoch in range(nEpochs):
        ...
        for batch in range(nBatches):
            ...
            yTrain = yTrain.type(torch.int64)
            ...
            for key in modelsDict.keys():
                ...
                if 'RNN' in modelsDict[key].name or 'rnn' in modelsDict[key].name or \
                    ...
                    xTrainOrdered = xTrainOrdered.view(batchSize[batch],seqLen,-1)
                ...
                # Compute loss
                lossValueTrain = modelsDict[key].loss(yHatTrain,yTrainModel.squeeze())
            ...
            if (epoch * nBatches + batch) % validationInterval == 0:
                ...
                yValid = yValid.type(torch.int64)
                ...
                for key in modelsDict.keys():
                    ...
                    if 'RNN' in modelsDict[key].name or 'rnn' in modelsDict[key].name or \
                        ...
                        xValidOrdered = xValidOrdered.view(nValid,seqLen,-1)
                    ...
                    with torch.no_grad():
                        ...
                        if 'GCRNN' in modelsDict[key].name or 'gcrnn' in modelsDict[key].name or \
                            ...
                            c0v = h0
                            yHatValid = modelsDict[key].archit(xValidOrdered.squeeze(),h0v,c0v)
                        else:
                            yHatValid = modelsDict[key].archit(xValidOrdered)

                       # Compute loss
                        lossValueValid = modelsDict[key]\
                                       .loss(yHatValid,yValidModel.squeeze())
                        ...
                        # Compute accuracy:
                        accValid = data.evaluate(yHatValid, yValidModel.squeeze())
                        ...
                        # Print:
                        if doPrint:
                            print("\t(%s) %6.2f %% / %6.2f - %6.4fs" % (
                                    key, 100*accValid , lossValueValid.item(),
                                    timeElapsed))
